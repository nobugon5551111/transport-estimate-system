...
const Step6Implementation = {
  ...
  displayServicesDetails: async () => {
    ...
    if (services.parking_officer_hours > 0 || services.parking_officer_cost > 0) {
      ...
      itemCalculations.push({ name: '駐車対策員', calculated: calculatedCost, saved: services.parking_officer_cost });
    ...
    if (details.length > 0) {
      ...
      if (services.total_cost && Math.abs(totalServicesCost - services.total_cost) > 1) {
        ...
        console.log('🔍 STEP6サービス項目別計算詳細:', itemCalculations);
        ...
        if (differences.length > 0) {
          console.log('⚠️ STEP6差異のある項目:', differences);
        }
    ...
    console.log('🔍 全サービス項目計算詳細:', itemCalculations);
