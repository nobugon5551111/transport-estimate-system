...
if (typeof MasterManagement === 'undefined') {
  ...
  displayProjectsContent: () => {
    console.log('案件マスターコンテンツを表示');
    // 案件データが既に存在する場合は表示を更新
    // データベースエラーが発生していても、タブは表示状態を維持
  ...
  displayProjectsTable: (projects) => {
    ...
    const html = projects.map(project => `
      <tr class="hover:bg-gray-50">
        <td class="px-6 py-4 whitespace-nowrap">
          <div class="text-sm font-medium text-gray-900">${project.name || '名称なし'}</div>
        </td>
        <td class="px-6 py-4 whitespace-nowrap">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
          <button onclick="MasterManagement.editProject('${project.id}')" class="text-blue-600 hover:text-blue-900 mr-3">
            <i class="fas fa-edit"></i> 編集
          </button>
          <button onclick="MasterManagement.deleteProject('${project.id}')" class="text-red-600 hover:text-red-900">
            <i class="fas fa-trash"></i> 削除
          </button>
        </td>
      </tr>
  ...
  openAddProjectModal: () => {
    ...
    MasterManagement.currentEditProjectId = null;
  ...
  createProjectModal: () => {
    // 既に存在する場合はスキップ
    if (document.getElementById('masterProjectModal')) return;
...
const CustomerManagement = {
  ...
  openAddProjectModal: () => {
    // 顧客選択肢を更新
    CustomerManagement.updateCustomerOptions();
