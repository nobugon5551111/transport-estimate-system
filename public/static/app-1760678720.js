...
console.log('🚀 JavaScript Version: 1760678720 - 動的ラベル更新機能追加済み（修正版）');
...
const Step4Implementation = {
  ...
  initialize: async () => {
    ...
    try {
      ...
      if (!flowData.customer || !flowData.project || !flowData.delivery || !flowData.vehicle) {
        ...
        console.log('⚠️ STEP4: セッションデータ不足ですが、マスター値ラベル更新は実行します');
        
        // HTML要素が存在するまで待機してから動的ラベル更新を実行（リトライ機能付き）
        setTimeout(async () => {
          console.log('🔄 遅延実行: HTML要素ロード後のマスター値ラベル更新開始');
    ...
    // マスター設定とラベル更新を実行（HTML要素準備完了後、リトライ機能付き）
    setTimeout(async () => {
      console.log('🔄 通常フロー: HTML要素ロード後のマスター値ラベル更新開始');
      await Step4Implementation.initializeMasterRates();
    }, 500);
  ...
  updateStaffPricingDisplay: (costs, rates, counts, totalCost, totalStaff) => {
    ...
    if (totalCost > 0) {
      ...
      // 各スタッフ種別の詳細表示（Step3の車両表示スタイル）
      if (counts.supervisorCount > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">スーパーバイザー ${counts.supervisorCount}人</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.supervisor.toLocaleString()}</span>
          </div>
        `;
      }
      ...
      if (counts.leaderCount > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">リーダー以上 ${counts.leaderCount}人</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.leader.toLocaleString()}</span>
          </div>
        `;
      }
      ...
      if (counts.m2HalfDay > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">M2スタッフ（半日）${counts.m2HalfDay}人</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.m2_half_day.toLocaleString()}</span>
          </div>
        `;
      }
      ...
      if (counts.m2FullDay > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">M2スタッフ（終日）${counts.m2FullDay}人</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.m2_full_day.toLocaleString()}</span>
          </div>
        `;
      }
      ...
      if (counts.tempHalfDay > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">派遣スタッフ（半日）${counts.tempHalfDay}人</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.temp_half_day.toLocaleString()}</span>
          </div>
        `;
      }
      ...
      if (counts.tempFullDay > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">派遣スタッフ（終日）${counts.tempFullDay}人</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.temp_full_day.toLocaleString()}</span>
          </div>
        `;
      }
      ...
      // 合計表示（Step3スタイル）
      htmlContent += `
            <div class="flex justify-between items-center py-3 px-4 bg-green-100 rounded border-2 border-green-300 mt-4">
              <span class="text-lg font-semibold text-gray-800">スタッフ費用合計</span>
              <span class="text-xl font-bold text-green-600">¥${totalCost.toLocaleString()}</span>
            </div>
          </div>
        </div>
      `;
      
      pricingDiv.innerHTML = htmlContent;
    } else {
  ...
  // 🔄 マスター設定取得と動的ラベル更新を実行する独立メソッド
  initializeMasterRates: async () => {
    try {
      console.log('🔄 Step4: マスター設定からスタッフ単価を取得中...');
      ...
      if (masterResponse.success && masterResponse.data && masterResponse.data.staff_rates) {
        ...
        Step4Implementation.staffRates = {
          ...
          leader_rate: masterStaffRates.leader || 30000,            // マスター値: 30,000円（最新）
          m2_half_day_rate: masterStaffRates.m2_half_day || 10000,  // マスター値: 10,000円（最新）
          m2_full_day_rate: masterStaffRates.m2_full_day || 20000,  // マスター値: 20,000円（最新）
          temp_half_day_rate: masterStaffRates.temp_half_day || 9000, // マスター値: 9,000円（最新）
          temp_full_day_rate: masterStaffRates.temp_full_day || 18000  // マスター値: 18,000円（最新）
        };
        
        console.log('✅ マスター設定からスタッフ単価を正常取得:', Step4Implementation.staffRates);
        
        // 🔄 マスターデータを入力フィールドと動的ラベルに自動設定
        Step4Implementation.setMasterRatesToInputFields(masterStaffRates);
        
        // 🔄 マスター設定取得後にスタッフ費用計算を実行
        setTimeout(() => {
          console.log('🔄 マスター設定取得後のスタッフ費用計算');
          Step4Implementation.updateStaffCost();
        }, 100);
      } else {
        console.warn('⚠️ マスター設定が空または取得失敗、最新マスター値ベースのデフォルト値を使用');
        Step4Implementation.staffRates = {
          supervisor_rate: 40000,  // 最新マスター値
          leader_rate: 30000,     // 最新マスター値
          m2_half_day_rate: 10000, // 最新マスター値
          m2_full_day_rate: 20000, // 最新マスター値
          temp_half_day_rate: 9000, // 最新マスター値
          temp_full_day_rate: 18000  // 最新マスター値
        };
        
        // 🔄 最新デフォルト値を入力フィールドと動的ラベルに自動設定
        Step4Implementation.setDefaultRatesToInputFields();
        
        // 🔄 デフォルト値設定後にスタッフ費用計算を実行
        setTimeout(() => {
          console.log('🔄 デフォルト値設定後のスタッフ費用計算');
          Step4Implementation.updateStaffCost();
        }, 100);
      }
    } catch (error) {
      console.error('❌ マスター設定取得エラー:', error);
      console.log('🔄 エラー時も最新マスター値ベースのデフォルトでラベル更新を実行');
      // エラー時も最新マスター値ベースのデフォルトを使用
      Step4Implementation.staffRates = {
        supervisor_rate: 40000,  // 最新マスター値
        leader_rate: 30000,     // 最新マスター値
        m2_half_day_rate: 10000, // 最新マスター値
      ...
      // 🔄 エラー時も最新デフォルト値を入力フィールドと動的ラベルに自動設定
      Step4Implementation.setDefaultRatesToInputFields();
      
      // 🔄 エラー時デフォルト値設定後にスタッフ費用計算を実行
      setTimeout(() => {
        console.log('🔄 エラー時デフォルト値設定後のスタッフ費用計算');
        Step4Implementation.updateStaffCost();
      }, 100);
    }
  },

  // 🔄 マスターデータを入力フィールドに自動設定する関数（リトライ機能付き）
  setMasterRatesToInputFields: (masterStaffRates, retryCount = 0, maxRetries = 10) => {
    console.log(`🔄 Step4: マスターデータを入力フィールド＆ラベル表示に自動設定開始 (試行回数: ${retryCount + 1}/${maxRetries + 1})`, masterStaffRates);
    
    try {
      // スタッフ単価の入力フィールドにマスターデータを設定
      const rateFields = [
        { id: 'supervisor_count', value: masterStaffRates.supervisor || 40000, displayId: 'rate-display-supervisor' },
        { id: 'leader_count', value: masterStaffRates.leader || 30000, displayId: 'rate-display-leader' },
        { id: 'm2_staff_half_day', value: masterStaffRates.m2_half_day || 10000, displayId: 'rate-display-m2-half' },
        { id: 'm2_staff_full_day', value: masterStaffRates.m2_full_day || 20000, displayId: 'rate-display-m2-full' },
        { id: 'temp_staff_half_day', value: masterStaffRates.temp_half_day || 9000, displayId: 'rate-display-temp-half' },
        { id: 'temp_staff_full_day', value: masterStaffRates.temp_full_day || 18000, displayId: 'rate-display-temp-full' }
      ];

      let elementsFound = 0;
      let totalElements = rateFields.length * 2; // 入力フィールド + 表示要素

      rateFields.forEach(field => {
        // 人数入力フィールドは更新しない（人数は0のままでよい）
        const inputElement = document.getElementById(field.id);
        if (inputElement) {
          console.log(`✅ ${field.id}: 入力要素を確認（値は更新しない）`);
          elementsFound++;
        } else {
          console.warn(`⚠️ ${field.id} 要素が見つかりません`);
        }
        ...
        if (displayElement) {
          ...
          console.log(`✅ ${field.displayId}: ${field.value.toLocaleString()}円 表示更新完了`);
          elementsFound++;
        } else {
          console.warn(`⚠️ ${field.displayId} 表示要素が見つかりません`);
        }
      });

      // 要素が見つからない場合はリトライ
      if (elementsFound === 0 && retryCount < maxRetries) {
        console.log(`🔄 HTML要素が見つからないため ${(retryCount + 1) * 1000}ms 後にリトライします...`);
        setTimeout(() => {
          Step4Implementation.setMasterRatesToInputFields(masterStaffRates, retryCount + 1, maxRetries);
        }, (retryCount + 1) * 1000);
        return;
      }

      if (elementsFound > 0) {
        console.log(`✅ Step4: マスターデータの入力フィールド＆ラベル表示自動設定完了 (${elementsFound}/${totalElements}個の要素を更新)`);
      } else {
        console.warn(`⚠️ Step4: すべてのHTML要素が見つかりませんでした。リトライ上限に達しました。`);
      }
    } catch (error) {
      console.error('❌ Step4: マスターデータ自動設定エラー:', error);
    }
  },

  // 🔄 デフォルト値を入力フィールドに設定する関数（リトライ機能付き）
  setDefaultRatesToInputFields: (retryCount = 0, maxRetries = 10) => {
    console.log(`🔄 Step4: デフォルト値を入力フィールド＆ラベル表示に設定開始 (試行回数: ${retryCount + 1}/${maxRetries + 1})`);
    
    try {
      const defaultRates = [
        { id: 'supervisor_count', value: 40000, displayId: 'rate-display-supervisor' },  // 最新マスター値
        { id: 'leader_count', value: 30000, displayId: 'rate-display-leader' },        // 最新マスター値
        { id: 'm2_staff_half_day', value: 10000, displayId: 'rate-display-m2-half' },  // 最新マスター値
        { id: 'm2_staff_full_day', value: 20000, displayId: 'rate-display-m2-full' },  // 最新マスター値
        { id: 'temp_staff_half_day', value: 9000, displayId: 'rate-display-temp-half' }, // 最新マスター値
        { id: 'temp_staff_full_day', value: 18000, displayId: 'rate-display-temp-full' } // 最新マスター値
      ];

      let elementsFound = 0;
      let totalElements = defaultRates.length * 2; // 入力フィールド + 表示要素

      defaultRates.forEach(field => {
        // 人数入力フィールドは更新しない（人数は0のままでよい）
        const inputElement = document.getElementById(field.id);
        if (inputElement) {
          console.log(`✅ ${field.id}: デフォルト入力要素を確認（値は更新しない）`);
          elementsFound++;
        } else {
          console.warn(`⚠️ ${field.id} 要素が見つかりません`);
        }
      ...
      if (elementsFound === 0 && retryCount < maxRetries) {
        console.log(`🔄 HTML要素が見つからないため ${(retryCount + 1) * 1000}ms 後にリトライします...`);
        setTimeout(() => {
          Step4Implementation.setDefaultRatesToInputFields(retryCount + 1, maxRetries);
        }, (retryCount + 1) * 1000);
        return;
      }

      if (elementsFound > 0) {
        console.log(`✅ Step4: デフォルト値の入力フィールド＆ラベル表示設定完了 (${elementsFound}/${totalElements}個の要素を更新)`);
      } else {
        console.warn(`⚠️ Step4: すべてのHTML要素が見つかりませんでした。リトライ上限に達しました。`);
      }
    } catch (error) {
      console.error('❌ Step4: デフォルト値設定エラー:', error);
    }
  }
};
...
const Step5Implementation = {
  ...
  initialize: async () => {
    ...
    try {
      ...
      if (masterResponse.success && masterResponse.data && masterResponse.data.service_rates) {
        ...
        // 🔄 マスターデータを入力フィールドに自動設定
        Step5Implementation.setMasterRatesToInputFields(masterRates);
        ...
        // 🔄 デフォルト値を入力フィールドに自動設定
        Step5Implementation.setDefaultRatesToInputFields();
      }
    } catch (error) {
      console.error('❌ マスター設定取得エラー:', error);
  ...
  updateServicesCost: () => {
    ...
    if (totalServicesCost > 0) {
      ...
      // 各サービス項目の詳細表示（Step3スタイル）
      if (costs.parking_officer > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">駐車対策員 ${parkingOfficerHours}時間</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.parking_officer.toLocaleString()}</span>
          </div>
        `;
      }
      ...
      if (costs.transport_vehicle > 0) {
        ...
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">人員輸送車両 ${transportVehicles}台（${distanceText}）</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.transport_vehicle.toLocaleString()}</span>
          </div>
        `;
      }
      
      if (costs.waste_disposal > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">引き取り廃棄（${wasteDisposal}）</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.waste_disposal.toLocaleString()}</span>
          </div>
        `;
      }
      
      if (costs.protection_work > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">養生作業 ${protectionFloors}フロア</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.protection_work.toLocaleString()}</span>
          </div>
        `;
      }
      
      if (costs.material_collection > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">残材回収（${materialCollection}）</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.material_collection.toLocaleString()}</span>
          </div>
        `;
      }
      
      if (costs.construction > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">施工 M2スタッフ${constructionM2Staff}人</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.construction.toLocaleString()}</span>
          </div>
        `;
      }
      ...
      if (costs.work_time_multiplier > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">作業時間帯割増（${workTimeType}）</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.work_time_multiplier.toLocaleString()}</span>
          </div>
        `;
      }
      
      if (costs.parking_fee > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">実費：駐車料金</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.parking_fee.toLocaleString()}</span>
          </div>
        `;
      }
      
      if (costs.highway_fee > 0) {
        htmlContent += `
          <div class="flex justify-between items-center py-2 px-4 bg-white rounded border border-gray-200">
            <span class="text-gray-700">実費：高速料金</span>
            <span class="text-lg font-medium text-gray-900">¥${costs.highway_fee.toLocaleString()}</span>
          </div>
        `;
      }
      ...
      // 合計表示（Step3スタイル）
      htmlContent += `
            <div class="flex justify-between items-center py-3 px-4 bg-blue-100 rounded border-2 border-blue-300 mt-4">
              <span class="text-lg font-semibold text-gray-800">その他サービス費用合計</span>
              <span class="text-xl font-bold text-blue-600">¥${totalServicesCost.toLocaleString()}</span>
            </div>
          </div>
        </div>
      `;
      
      costDisplay.innerHTML = htmlContent;
    } else {
  ...
  // 🔄 マスターデータを入力フィールドに自動設定する関数
  setMasterRatesToInputFields: (masterRates) => {
    console.log('🔄 Step5: マスターデータをラベル表示に自動設定開始', masterRates);
    
    try {
      // Step5では表示ラベルのみを更新（入力フィールドの設定は不要）
      const displayLabels = [
        { displayId: 'rate-display-parking-officer', value: masterRates.parking_officer_hourly || 2500 },
        { displayId: 'rate-display-transport-20km', value: masterRates.transport_20km || 15000 },
        { displayId: 'rate-display-transport-km', value: masterRates.transport_per_km || 150 },
        { displayId: 'rate-display-fuel', value: masterRates.fuel_per_liter || 160 },
        { displayId: 'rate-display-waste-small', value: masterRates.waste_small || 10000 },
        { displayId: 'rate-display-waste-medium', value: masterRates.waste_medium || 15000 },
        { displayId: 'rate-display-waste-large', value: masterRates.waste_large || 20000 },
        { displayId: 'rate-display-material-few', value: masterRates.material_few || 5000 },
        { displayId: 'rate-display-material-medium', value: masterRates.material_medium || 10000 },
        { displayId: 'rate-display-material-many', value: masterRates.material_many || 15000 },
        { displayId: 'rate-display-protection-base', value: masterRates.protection_base || 8000 },
        { displayId: 'rate-display-protection-floor', value: masterRates.protection_floor || 3000 },
        { displayId: 'rate-display-construction-m2', value: masterRates.construction_m2_staff_rate || 0 }
      ];

      displayLabels.forEach(label => {
        const displayElement = document.getElementById(label.displayId);
        if (displayElement) {
          displayElement.textContent = label.value.toLocaleString();
          console.log(`✅ ${label.displayId}: ${label.value.toLocaleString()}円 表示更新完了`);
        } else {
          console.warn(`⚠️ ${label.displayId} 表示要素が見つかりません`);
        }
      });

      console.log('✅ Step5: マスターデータのラベル表示自動設定完了');
    } catch (error) {
      console.error('❌ Step5: マスターデータ自動設定エラー:', error);
    }
  },

  // 🔄 デフォルト値をラベル表示に設定する関数
  setDefaultRatesToInputFields: () => {
    console.log('🔄 Step5: デフォルト値をラベル表示に設定開始');
    
    try {
      // Step5では表示ラベルのみを更新（入力フィールドの設定は不要）
      const defaultDisplayLabels = [
        { displayId: 'rate-display-parking-officer', value: 2500 },
        { displayId: 'rate-display-transport-20km', value: 15000 },
        { displayId: 'rate-display-transport-km', value: 150 },
        { displayId: 'rate-display-fuel', value: 160 },
        { displayId: 'rate-display-waste-small', value: 10000 },
        { displayId: 'rate-display-waste-medium', value: 15000 },
        { displayId: 'rate-display-waste-large', value: 20000 },
        { displayId: 'rate-display-material-few', value: 5000 },
        { displayId: 'rate-display-material-medium', value: 10000 },
        { displayId: 'rate-display-material-many', value: 15000 },
        { displayId: 'rate-display-protection-base', value: 8000 },
        { displayId: 'rate-display-protection-floor', value: 3000 },
        { displayId: 'rate-display-construction-m2', value: 0 }
      ];

      defaultDisplayLabels.forEach(label => {
        const displayElement = document.getElementById(label.displayId);
        if (displayElement) {
          displayElement.textContent = label.value.toLocaleString();
          console.log(`✅ ${label.displayId}: ${label.value.toLocaleString()}円 表示更新完了`);
      ...
      console.log('✅ Step5: デフォルト値のラベル表示設定完了');
    } catch (error) {
      console.error('❌ Step5: デフォルト値設定エラー:', error);
    }
  }
};
...
// DOMContentLoaded時にボタン設定を実行（Step4でのみ）
document.addEventListener('DOMContentLoaded', function() {
  // Step4画面でのみプロジェクトボタンを設定
  if (document.getElementById('addProjectBtn') || document.getElementById('projectForm')) {
    console.log('🚀 Setting up project button on DOMContentLoaded');
    setupProjectAddButton();
  }
});

// window.onloadでも設定（念のため・Step4でのみ）
